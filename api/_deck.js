// Deck generation, server-side only. This module never reaches the browser,
// so the prompt and the API key stay private.
//
// Used by both the Vercel function (api/generate.js) and the dev server
// middleware (vite.config.js), so local and deployed behaviour cannot drift.

const CARD_TARGET = 10;
const QUIZ_TARGET = 5;
const LONG_NOTES_WORDS = 3000;
...
export function normalizeDeck(raw, notes) {
  const warnings = [];
  ...
  if (!flashcards.length && !quiz.length) {
    throw new DeckError(502, 'The model did not return any usable cards or questions.');
  }
  if (!quiz.length) {
...
export async function generateDeckFromNotes(notes, { apiKey, model }) {
  if (!apiKey) {
    throw new DeckError(500, 'The server has no Groq API key configured.');
  }
  ...
  try {
    ...
    throw new DeckError(502, 'Could not reach the model. Try again in a moment.');
  }

  if (response.status === 401 || response.status === 403) {
    throw new DeckError(500, "The server's Groq key was rejected.");
  }
  if (response.status === 429) {
    throw new DeckError(429, 'The model is rate limiting right now. Wait a moment and try again.');
  }
  if (!response.ok) {
    throw new DeckError(502, `The model request failed (${response.status}).`);
  }

  const payload = await response.json().catch(() => null);
  const content = payload?.choices?.[0]?.message?.content;
  if (!content) {
    throw new DeckError(502, 'The model returned an empty response. Try again.');
  }
  ...
  try {
    ...
    throw new DeckError(502, 'The model returned something that was not valid JSON. Try again.');
  }
